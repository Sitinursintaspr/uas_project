  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
     ...
     $posisi = $p->cariPosisi($batas);
      ...
      echo "<br><center><h3>Pengelolaan Data Siswa</h3><hr></center>
          <input class='button blue' type=button value='Tambah' onclick=\"window.location.href='?module=siswa&act=tambahsiswa';\">";
      

      echo "<br><br><div class='information msg'>Siswa tidak bisa di hapus, namun bisa di non aktifkan.</div>";
      echo "<br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>NIS</th><th>Nama</th><th>Kelas</th><th>Jenis Kelamin</th>
            <th>Non-Aktif</th><th>Opsi</th></tr></thead>";
      $no = $posisi+1;
    while ($r=mysql_fetch_array($tampil_siswa)){
       echo "<tr><td>$no</td>
             ...
             <td><a href='?module=siswa&act=editsiswa&id=$r[id_siswa]' title='Edit'><img src='images/icons/edit2.png' alt='Edit' /></a> |

                 <a href=?module=detailsiswa&act=detailsiswa&id=$r[id_siswa] title='Detail'><img src='images/icons/detail1.png' alt='Edit' /></a></td></tr>";
      $no++;
    }
    echo "</table>";
...
case "lihatmurid":
    ...
    $cek_siswa = mysql_num_rows($tampil);
    if(!empty($cek_siswa)){
    echo"<br><center><h2>Daftar Teman</b><br><p class='garisbawah'></h2></center>";
    echo "<table>
...
case "tambahsiswa":
    ...
    echo "</dd>
          ...
          <dt><label>Aktif</label></dt>       <dd> : <label><input type=radio name='blokir' value='Y'> Y</label>
  ...
  case "editsiswa":
    ...
    echo "<form method=POST action=$aksi?module=siswa&act=update_siswa enctype='multipart/form-data'>
          ...
          <fieldset>
          <center><h1>Ganti Data Siswa</h1></center>
          <dl class='inline'>
    ...
    if ($r[blokir]=='N'){
      echo "<dt><label>Aktif</label></dt>     <dd> : <label><input type=radio name='blokir' value='Y'> Y</label>
                                           <label><input type=radio name='blokir' value='N' checked> N </label></tr>";
    }
    else{
      echo "<dt><label>Aktif</label></dt>     <dd> : <label><input type=radio name='blokir' value='Y' checked> Y</label>
    ...
    elseif ($_SESSION[leveluser]=='siswa') {
     echo"<br><center><h1>Ganti Data Siswa</h1></center><br><p class='garisbawah'></p>";
 ...
 case "detailsiswa":
    if ($_SESSION[leveluser]=='admin'){
       ...
       echo "<form><fieldset>
          <h1>Data Siswa MTS Tarbiyatul Falah</h1>
          <dl class='inline'>
          ...
          <dt><label>Aktif</label></dt>             <dd> : $siswa[blokir]</dd>
          <dt><label>Foto</label></dt>             <dd> :
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
       ...
       <table id='table1' class='gtable sortable'>
          ...
          <tr><td>Opsi</td>               <td> : <input type=button class='button small white' value=Kembali onclick=self.history.back()></td></tr>";
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
      ...
      echo"<br><b class='judul'>Data Siswa MTS Tarbiyatul Falah</b><br><p class='garisbawah'></p>
       <table align='center'>
...
case "detailprofilsiswa":
    if ($_SESSION[leveluser]=='siswa'){
      ...
      echo"<br><h1><center>Data Siswa MTS Tarbiyatul Falah</center></h1><br><p class='garisbawah'></p>
       <table>
             <tr><td rowspan='14'>";if ($siswa[foto]!=''){
              echo "<img src='foto_siswa/medium_$siswa[foto]'>";
          }
          echo "
          </td><td>NIS</td>               <td> : $siswa[nis]</td></tr>
          <tr><td>Nama</td>               <td> : $siswa[nama_lengkap]</td></tr>
          <tr><td>Kelas</td>              <td> : $kelas[nama]</td></tr>
          <tr><td>alamat</td>             <td> : $siswa[alamat]</td></tr>
          <tr><td>Tempat Lahir</td>       <td> : $siswa[tempat_lahir]</td></tr>
...
case "detailaccount":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        echo"<br><center><h1>Ganti Akun Login</h1></center><br><p class='garisbawah'></p>
        <center>
        <tr><td>Username</td><td>:<input type=text name='username' size='40' placeholder='username'></td></tr>
        <tr><td>Password</td><td>:<input type=password name='password' size='40'placeholder='password'></td></tr>
        <tr><td colspan=2>Catatan :</td></tr>
        <tr><td colspan=2>*) Apabila Username tidak diubah kosongkan saja.</td></tr>
        <tr><td colspan=2>**) Apabila Password tidak diubah kosongkan saja.</td></tr>
